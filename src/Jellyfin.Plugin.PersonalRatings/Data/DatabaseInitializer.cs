...
internal sealed class DatabaseInitializer : IDatabaseInitializer
{
    private const string InitializationSql = """
        ...
        CREATE INDEX IF NOT EXISTS idx_tag_definitions_enabled_sort
            ON tag_definitions(is_enabled, sort_order, name);
        ...
        CREATE INDEX IF NOT EXISTS idx_user_item_tags_user_item
            ON user_item_tags(user_id, item_id);

        CREATE INDEX IF NOT EXISTS idx_user_item_tags_user_tag
            ON user_item_tags(user_id, tag_id);
        """;
