...
- 支持待删除标记与取消待删除
- 支持全局标签定义与用户条目标签关系
- 提供前台“打分库”浏览页，当前已按 Jellyfin 原生媒体库页结构收口，支持海报视图 / 列表视图切换，并使用轻量弹出式筛选面板
- 提供评分后台页、标签管理页与删除审计页
- 评分后台页支持多选后批量添加标签 / 批量移除标签
- 评分后台页支持按启用标签进行多选筛选，并支持 `any / all` 标签匹配
- 支持分页、筛选、多选和批量操作
- 只有管理员可以调用物理删除接口
...
- “打分库”是主浏览入口，当前尽量复用 Jellyfin 媒体库页的工具条、卡片流、列表行和暗黑主题结构，只轻量叠加评分 / 标签 / 待删除维度
...
- 标签管理页负责维护标签定义
- 评分后台页负责按标签筛选当前列表，并把标签批量应用到当前用户选中的条目

本轮改动重点：

- 继续沿用现有 API 契约
- 主要收口评分后台里的标签筛选、批量标签提示、空状态和操作反馈
- 不扩展前台“打分库”范围，不新增新的产品概念

## 本轮前端结构

当前前端仍保持轻量原生脚本，并且前台“打分库”已从大块自绘页收口为“原生媒体库页增强层”。现有模块按职责拆分为：
