...
4. 验证前台浏览页能正常加载为接近 Jellyfin 原生媒体库页的卡片流，而不是空白页或大块自绘面板。
5. 点开搜索 / 排序 / 筛选入口，确认它们以轻量弹出面板方式出现，而不是常驻大块表单。
6. 切换海报视图 / 列表视图，确认页面能正常切换，且结果区域会尽量铺满可用宽度，不会在右侧留下明显大块空白。
7. 试用评分、播放状态、类型、排序、搜索等筛选。
8. 如果已经存在标签定义，确认顶部标签 chips 能加载并参与筛选，且卡片只显示 2~3 个标签摘要。
9. 点击任意卡片，确认能跳回原始 Jellyfin 详情页。
10. 在详情页确认出现统一操作区，并验证：
   - 打分
   - 待删除
   - 标签切换交互
11. 打开评分后台：
...
12. 验证分页加载、批量操作和后台入口仍可用。
13. 在评分后台里勾选多个条目，打开“批量标签”面板，验证：
    - 当前已选条目数显示正确
    - 已启用标签能正常加载
    - 支持多选标签
    - “批量添加标签”与“批量移除标签”有明确区分
    - 批量添加标签 / 批量移除标签后列表标签显示会刷新
    - 如果当前没有标签定义，会显示明确提示而不是无响应
13. 在评分后台顶部验证标签筛选：
    - 已启用标签 chips 能正常加载
    - 点击标签后列表会按标签刷新
    - 同时选中多个标签时可切换 `匹配任一标签 / 必须同时包含`
    - 当前生效的标签、匹配方式和其他筛选条件摘要清晰可见
    - 清空标签筛选后列表会恢复当前预设下的普通结果集
14. 打开标签管理页并验证管理员能力：
...
15. 验证标签列表、新建、编辑、启用 / 停用、排序号和删除都可用。
16. 使用管理员账号验证物理删除。
17. 打开删除审计页并确认能看到刚才的删除记录：

    ```text
    #/configurationpage?name=PersonalRatingsAuditPage
    ```

18. 确认数据库中出现 `delete_audit_logs` 记录。
...
另外，前台“打分库”虽然已经尽量贴近 Jellyfin 原生媒体库页结构，但它仍然是插件增强层，不是 Jellyfin 内建页面；如果页面看起来像旧版本，优先先看静态资源是否已经刷新。
