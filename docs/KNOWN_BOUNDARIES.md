...
当前主入口虽然已经从配置页转到前台顶栏，并且页面结构也尽量向 Jellyfin 原生媒体库页靠拢，但它仍然不是 Jellyfin 内建的原生模块，而是通过注入脚本挂到现有 Web 壳结构上。
...
- 前台浏览页标签筛选 chips
- 标签管理后台页
- 评分后台页里的批量添加标签 / 批量移除标签

当前仍未完全收口：

- 标签定义目前是全局共享，而不是按用户隔离
- 前台卡片、列表行、工具条与轻量筛选弹层虽然已经按 Jellyfin 媒体库页结构收口，但仍是注入式 MVP 实现
- 标签管理页仍是基础 CRUD 版本，没有更复杂的批量编排与拖拽排序
- 评分后台页的批量标签闭环已可用，但仍是当前页多选 + 重新拉取列表的保守实现
...
- 让标签筛选、空状态、批量标签提示和反馈更清晰
- 不新增新的标签能力概念
- 不改变现有接口契约和查询策略

但这条链路仍然刻意保持保守：

- 只作用于当前后台页查询结果，不做跨页持久选集
- 标签操作完成后优先重新拉取当前列表，而不是做复杂的前端乐观更新
- 标签筛选只使用当前启用中的标签定义，不提供更复杂的分组、拖拽或条件组合
